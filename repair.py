...
def progressive_inpaint(img, defect_params, max_iters=5):
    ...
    for _ in range(max_iters):
        ...
        edges = cv2.dilate(edges, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)))  # 解决边缘丢失
        mask = cv2.dilate(mask, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)))  # 逐层扩展
